...
export const MobileNav: FC<{ escolas: Escola[] }> = (props) => {
  ...
  return (
    <div className="md:hidden select-none flex h-14 w-full bg-blue-500 fixed bottom-0 left-0 z-50 shadow-md shadow-gray-400">
      <div
        onClick={() =>
          navigate("/login", { state: { escolas: props.escolas } })
        }
      ...
      <div
        onClick={() =>
          navigate("/signup", { state: { escolas: props.escolas } })
        }
        className={`w-1/3 flex h-full ${pathname == '/signup' ? 'bg-[#488cf9]' : 'bg-blue-500'} border-l border-blue-600`}
      >
        <h2 className="my-auto w-full text-white font-sans font-semibold text-center text-sm">
      ...
      <div
        onClick={() =>
          navigate("/search", { state: { escolas: props.escolas } })
        }
        className={`w-1/3 flex h-full ${pathname == '/search' ? 'bg-[#488cf9]' : 'bg-blue-500'} border-l border-blue-600`}
      >
        <h2 className="my-auto w-full text-white font-sans font-semibold text-center text-sm">
